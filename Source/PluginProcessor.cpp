...
void SimpleEQAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    spec.numChannels=1; //mono chain has only one channel
    ...
    switch (chainSettings.lowCutSlope)
    {
        ...
        case Slope_48:
            *leftLowCut.get<0>().coefficients=*cutCoefficients[0];
            leftLowCut.setBypassed<0>(false);*leftLowCut.get<1>().coefficients=*cutCoefficients[1];
            leftLowCut.setBypassed<1>(false);*leftLowCut.get<2>().coefficients=*cutCoefficients[2];
            leftLowCut.setBypassed<2>(false);*leftLowCut.get<3>().coefficients=*cutCoefficients[3];
    ...
    switch (chainSettings.lowCutSlope)
    {
        ...
        case Slope_48:
            *rightLowCut.get<0>().coefficients=*cutCoefficients[0];
            rightLowCut.setBypassed<0>(false);*rightLowCut.get<1>().coefficients=*cutCoefficients[1];
            rightLowCut.setBypassed<1>(false);*rightLowCut.get<2>().coefficients=*cutCoefficients[2];
            rightLowCut.setBypassed<2>(false);*rightLowCut.get<3>().coefficients=*cutCoefficients[3];
...
ChainSettings getChainSettings(juce::AudioProcessorValueTreeState& apvts)
{
    ...
    // apvts.getParameter("LowCut Freq")->getValue(); expects real world values not normalized values
    ...
    SimpleEQAudioProcessor::createParameterLayout()
    {
        ...
        layout.add(
            std::make_unique<juce::AudioParameterFloat>(
                "LowCut Freq","LowCut Freq", //name and param name
                juce::NormalisableRange<float>(20.f,20000.f,1.f,0.25f), //range 20 to 20000 then slider increment and skew(fls mixer concept)
                20.f //default value for low cut
            )
        );
        layout.add(
            std::make_unique<juce::AudioParameterFloat>(
